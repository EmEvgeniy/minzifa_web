...
export function ConsultationQuiz({ popupClose }: ConsultationQuizFormProps) {
    ...
    const validateField = (fieldName: keyof ConsultationQuizFormRequest, value: string): string | null => {
    ...
    const { mutate, isPending } = usePostMutation<ConsultationQuizFormRequest, ConsultationQuizFormRequest>(
        ['create-my-trip-quiz'],
        () => {
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white">
            <div className="bg-gray-100 w-full p-3 md:p-5">
                <h2 className="w-full pr-20 text-lg font-semibold">Заполните данные чтобы получить &quot;Бесплатную консультацию&quot; от эксперта по путешествиям</h2>
            </div>
            {progress && (
                    ...
                    {questions?.map((question, index) => index === step - 1 && (
                        ...
                        >
                            ...
                            {(question.type === 'checkbox' || question.type === 'radio') && (
                                <>
                                    <div className={cn("grid gap-5", question?.answers?.length && question?.answers?.length > 3 ? 'grid-cols-2' : 'grid-cols-1')}>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 items-center text-center md:text-left gap-5 bg-gray-100 p-3 md:p-5'>
                ...
                <div className='grid grid-cols-2 gap-5'>
                    ...
                    {step === questions?.length && <button type="submit" disabled={isPending} className='bg-[#3BA151] text-white p-2 rounded-lg w-full cursor-pointer'>{t('buttons.submit')}</button>}
                </div>
            </div>
        </form>
    );
