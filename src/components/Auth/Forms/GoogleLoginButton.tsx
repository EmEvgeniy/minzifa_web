...
export function GoogleLoginButton() {
    ...
    const onGoogleLogin = async () => {
        try {
            ...
            if (errorMessage === "Авторизация отменена") {
    ...
    return (
        <div className="w-full">
            <button
                ...
                disabled={isLoading}
                className="cursor-pointer flex items-center justify-center gap-3 w-full px-4 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                type="button"
            >
                {isLoading ? (
                    <span className="text-gray-600">{t("googleLoading")}</span>
                ) : (
                    <>
                        <svg
                            className="w-5 h-5"
