...
export const Orders = () => {
  ...
  const {
    ...
    isFetched,
    isLoading,
  } = useAuthGetQuery<PaginatedForms>({
    key: ['orders'],
    page: page,
    perPage: '10',
    url: 'auth/orders',
  ...
  return (
    isFetched && (
      <div className="container mb-[50px] mx-auto px-4 py-8">
        <div className="mb-8">
        ...
        {forms?.data && forms.data.length > 0 ? (
          <div className="space-y-4">
            {forms.data.map((form) => {
              ...
              return (
                <Link href={`/${locale}/orders/${form.id}`} key={form.id} className="block">
                  <div className="bg-white rounded-xl shadow-md border border-gray-100 p-5 md:p-6 transition-all duration-300 hover:shadow-xl hover:border-green-200 hover:-translate-y-1">
                    <div className="flex justify-between items-start mb-4 pb-4 border-b border-gray-100">
                      ...
                      <div
                        className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium ${getStatusColor(formData.payment_status)}`}
                      >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm font-medium text-gray-900 truncate">
                            {mainPassenger
                              ? `${mainPassenger.first_name || ''} ${mainPassenger.last_name || ''}`.trim()
                              : '-'}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100">
                      <div className="flex items-center text-xs text-gray-500">
                        ...
                        {t('orders.created')}: {new Date(form.created_at).toLocaleDateString()}{' '}
