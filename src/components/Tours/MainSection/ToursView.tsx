...
export const ToursView = () => {
  const { price } = useFilterStore((state) => state);
  ...
  const { data, isSuccess, isLoading } = useGetQuery<{ data: AllToursCardType[] }>({
    key: ['all_tours', value, `${price[0]}`, `${price[1]}`],
    page: String(page),
    ...
    <div className="w-full flex flex-col gap-5 items-start justify-start">
      <div className="w-full flex items-center justify-between min-h-[57px] [@media(max-width:1024px)]:justify-end">
        <p className="block [@media(max-width:1024px)]:hidden">
          {t('all_tours.showing')} {page} - {totalPages} {t('all_tours.out')} {data?.data?.length}
        </p>
        <p
          onClick={handleClick}
