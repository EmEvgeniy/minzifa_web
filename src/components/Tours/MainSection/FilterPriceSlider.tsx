...
function FilterPriceSlider({ pl, pl2, pl3 }: { pl: string; pl2: string; pl3: string }) {
  ...
  const minVal = prices[0] || 0;
  const maxVal = prices[1] || 20000;

  const getPercent = (value: number) => (value / 20000) * 100;
  ...
  const handleSliderClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const clickValue = (clickPercent / 100) * 20000;
  ...
  return (
    <FilterAccordion title={pl} filterKey="price">
