...
export default function TourPrices({ tour }: { tour: Tour }) {
  ...
  const { setBookingData, bookingData } = useBookingStore((state) => state);
  ...
  return tour.prices.length ? (
    ...
    <section className="w-full flex items-center justify-start py-10 flex-col gap-5">
      <div className="bg-white shadow-xl p-5 rounded-[16px] flex flex-col w-full max-w-[760px] gap-5 items-center">
        ...
        <Button
          onClick={() => {
            setBookingData({
              ...bookingData,
              travellers_count: bookingData.travellers_count,
              payment_type: 'cash',
              payment_status: 'pending',
            });
            const params = new URLSearchParams({
              tour_name: tour.name,
              tour_start: bookingData.tour_start || '',
              tour_end: bookingData.tour_end || '',
              travellers_count: bookingData.travellers_count?.toString() || '1',
              tour_price: '',
              deposit: '',
              total_price: '',
              payment_type: 'cash',
              ...
              single_price: '',
              currency: 'USD',
              total_seats: bookingData.total_seats?.toString() || '1',
            });
            router.push(`/${locale}/booking/${tour?.slug}?${params.toString()}`);
          }}
          fullWidth
