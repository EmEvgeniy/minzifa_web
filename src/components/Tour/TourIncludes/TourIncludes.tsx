...
export const TourIncludes = ({ includes }: { includes: Include[] | undefined }) => {
    ...
    return (
        <div className="flex flex-col gap-5 col-start-1 ">
            <h2 className="text-4xl font-semibold text-black">{t('includes.title')}</h2>
            {[includeItems, excludeItems].map((item, index) => (
                <div className="bg-white rounded-2xl" key={index}>
                    <h2 className="text-2xl font-semibold text-black p-5">{index === 0 ? t("includes.include") : t("includes.exclude")}</h2>
                    <CustomAccordion
                        expandedIndexes={index === 0 ? includeAccordionIndexes : excludeAccordionIndexes}
                        onExpandedIndexesChange={index === 0 ? setIncludeAccordionIndexes : setExcludeAccordionIndexes}
                    >
                        {item?.map((include) => (
                            <div key={include.id}>
            ...
            <div className="bg-[#E2FFF4] p-5 rounded-2xl flex flex-row gap-2.5 items-center text-lg">
