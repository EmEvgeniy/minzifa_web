...
import { Dropdown, DropdownDetails, DropdownItem, DropdownSummary } from "@/components/UI/Dropdown/Dropdown";
import { useMemo } from "react";
import IconInfo from "../../../assets/icons/booking/exclamationmark.circle.svg";
import Image from "next/image";

const InputField = ({ value, onChange, placeholder }: { value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; placeholder: string }) => (
    <input
        type="text"
        value={value}
...
export const Passenger = ({ index }: { index: number }) => {
    ...
    const months: { [key: string]: string[] } = {
        ...
        ru: ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь"],
    };
    ...
    return (
        <div className="space-y-6">
            <div className="bg-[#C5DCD3]/50 rounded-2xl px-2.5 py-5 flex flex-row items-center gap-2">
                <Image
                    src={IconInfo}
                    alt={""}
                    width={30}
            ...
            <div className="mt-2 space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                    <InputField
                        ...
                        onChange={(e) => updatePassengerField(index, "main_address.postal_code", e.target.value)}
