...
export default async function generateSitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    for (const locale of LOCALES) {
        const [tourRes, destinationRes, articleRes] = await Promise.all([
            fetch(`https://api.minzifatravel.com/api/v1/tours?all=1&locale=${locale}`, { next: { revalidate: 3600 } }),
            fetch(`https://api.minzifatravel.com/api/v1/destinations?all=1&locale=${locale}`, { next: { revalidate: 3600 } }),
            fetch(`https://api.minzifatravel.com/api/v1/articles?all=1&locale=${locale}`, { next: { revalidate: 3600 } }),
        ]);

        toursByLocale[locale] = await tourRes.json();
        destinationByLocale[locale] = await destinationRes.json();
        articleByLocale[locale] = await articleRes.json();
    }
